...
//    UDP    /////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
#define SYUDPSOCKET_BUFFER_LENGTH	1024
#define SYUDPBROADCAST_KEY	L"SYBROADCAST_V1.00.00"
enum SYUDPSocketEventStatus
{
	SYUDPSOCKET_CONNECTFAULT = 0,
	SYUDPSOCKET_LISTENED,
	SYUDPSOCKET_RECVDATA,
	SYUDPSOCKET_FIND_SERVICE
	/*
	SYTCPSOCKET_CLOSE = 0,
	SYTCPSOCKET_CONNECTFAULT,
	SYTCPSOCKET_CONNECTED,
...
struct SYUDPEvent
{
	SYUDPSocketEventStatus	Status;
	wchar_t					wszMsg[256];
	char					szData[SYUDPSOCKET_BUFFER_LENGTH];
	USHORT					Port;
	int						iLen;
};
//////////////////////////////////////////////////////////////////////////
DWORD WINAPI BroadcastServiceProcThread(void *pParam);
DWORD WINAPI BroadcastClientProcThread(void *pParam);
//////////////////////////////////////////////////////////////////////////
class SYUDPSocket {
public:
	//Server
	...
	char	_szIP[16];
	long    _timerOut;
	HANDLE _hScanProcThread;
	//Common
	SOCKET  Socket;
	//Event
	void (*OnEvent)(SYUDPSocket *sender, SYUDPEvent e);
	...
	void Close()
	{
		...
		if (_hScanProcThread)
		{
			TerminateThread(_hScanProcThread, 0);
			_hScanProcThread = NULL;
		}
	...
	void StartListenWithPort(USHORT port) {
		if (flag_IsListen){ return;}
		this->Close();
		LocalPort = port;		

		//Listen
		_hListenProcThread = CreateThread(NULL, 0, BroadcastServiceProcThread, this, 0, NULL);
	}
	...
	void StartBroadcastWithHost(char *ip, USHORT port, long timerOut)
	{
		this->Close();

		memset(_szIP, 0, sizeof(_szIP));		
		sprintf_s(_szIP, "%s", ip);
		LocalPort = port;
		_timerOut = timerOut;
		
		//Scan
		_hScanProcThread = CreateThread(NULL, 0, BroadcastClientProcThread, this, 0, NULL);		
	}
};
...
DWORD WINAPI BroadcastServiceProcThread(void *pParam)
{
	SYUDPSocket *pSYSocket = (SYUDPSocket*)pParam;	
	...
	unsigned long int name_size=255;

	if(!GetComputerName(userName, &name_size))
	{
		memset(userName, 0, sizeof(userName));
		wsprintf(userName, L"%s", L"undefine");
	}
	...
	if (iResult != NO_ERROR) {
		...
		SYUDPEvent e;
		ZeroMemory(&e, sizeof(e));
		e.Status = SYUDPSOCKET_CONNECTFAULT;
		wsprintf(e.wszMsg, L"%s", L"Error at WSAStartup()\n");
		pSYSocket->OnEvent(pSYSocket, e);
		return false;
	}			
	...
	// Create a SOCKET for listening for
	// incoming connection requests.		
	pSYSocket->Socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if (pSYSocket->Socket == INVALID_SOCKET) 
	{
		memset(wszBuf, 0, sizeof(wszBuf));
		wsprintf(wszBuf, L"Error at socket(): %ld\n", WSAGetLastError());
	...
	SYUDPEvent e;
	ZeroMemory(&e, sizeof(e));
	e.Status = SYUDPSOCKET_LISTENED;
	wsprintf(e.wszMsg, L"%s", wszBuf);
	pSYSocket->OnEvent(pSYSocket, e);

	pSYSocket->flag_IsListen = true;
	while (true)
	{		

		char recv_data[SYUDPSOCKET_BUFFER_LENGTH];   

		SOCKADDR_IN client_addr;
		int len = sizeof(client_addr);
		int bytes_recieved = recvfrom(pSYSocket->Socket, recv_data, SYUDPSOCKET_BUFFER_LENGTH, 0, (sockaddr*)&client_addr, &len) ;  
		if (bytes_recieved>0){
			recv_data[bytes_recieved] = '\0';
		}
		...
		{	
			...
			SYUDPEvent e;
			ZeroMemory(&e, sizeof(e));
			e.Status = SYUDPSOCKET_RECVDATA;
			e.iLen = bytes_recieved;
			memcpy(e.szData, recv_data, sizeof(recv_data) );				
			e.Port = ntohs(client_addr.sin_port);			
			pSYSocket->OnEvent(pSYSocket, e);	


			// ANSI(char) to Unicode(WCHAR)
			wchar_t *pUnicode = NULL;
			AnsiToUnicode(recv_data, &pUnicode);

			if (wcscmp(SYUDPBROADCAST_KEY, pUnicode) == 0)
			{
				wchar_t msg[512];
				memset(msg, 0, sizeof(msg));
				wsprintf(msg, L"<pack><key>%s</key><name>%s</name></pack>", SYUDPBROADCAST_KEY, userName);

				// Unicode(WCHAR) to ANSI(char)
				char *pAnsi = NULL;
				UnicodeToAnsi(msg, &pAnsi);
				//Resp to client
				sendto(pSYSocket->Socket, (const char*)pAnsi, (int)strlen(pAnsi), 0, (SOCKADDR*)&client_addr, sizeof(client_addr));
				free(pAnsi);
				
				char szBuf[256];
				memset(szBuf, 0, sizeof(szBuf));
				sprintf_s(szBuf, "\nClient to Server(%s , %d) said : %s\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port), e.szData);
	...
	shutdown(pSYSocket->Socket, SD_BOTH);
	closesocket(pSYSocket->Socket);
	WSACleanup();
	OutputDebugStringA("UDP Server is Terminate\n");
...
DWORD WINAPI BroadcastClientProcThread(void *pParam)
{
	...
	struct hostent *pHostEnt = gethostbyname(pSYSocket->_szIP);    
	struct in_addr in;
	in.s_addr   =((struct in_addr*)pHostEnt->h_addr)->s_addr;
	char *pAddress = inet_ntoa(in);
	nMyIP       = inet_addr(pAddress);
	...
	// The sockaddr_in structure specifies the address family,
	// IP address, and port for the socket that is being bound.	
	sockaddr_in boradcast_addr;
	memset(&boradcast_addr, 0, sizeof(boradcast_addr));
	boradcast_addr.sin_family = AF_INET;
	boradcast_addr.sin_addr.s_addr = nTemp1 + nTemp2;	
	boradcast_addr.sin_port = htons(pSYSocket->LocalPort);
	
	int so_broadcast = TRUE;
	if (setsockopt(pSYSocket->Socket, SOL_SOCKET, SO_BROADCAST, (char*)&so_broadcast, sizeof(so_broadcast)) == SOCKET_ERROR)
	{
		memset(wszBuf, 0, sizeof(wszBuf));
		wsprintf(wszBuf, L"setsockopt() error: %ld\n", WSAGetLastError());
		SYUDPEvent e;
		ZeroMemory(&e, sizeof(e));
		e.Status = SYUDPSOCKET_CONNECTFAULT;
		wsprintf(e.wszMsg, L"%s", wszBuf);
	...
	//set Timeout	
	struct timeval tv;
	tv.tv_sec = pSYSocket->_timerOut;  //2s
	tv.tv_usec = 0; //500000 = 0.5s
	if (setsockopt(pSYSocket->Socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof(tv)) == SOCKET_ERROR)
	{
		memset(wszBuf, 0, sizeof(wszBuf));
		wsprintf(wszBuf, L"setsockopt() error: %ld\n", WSAGetLastError());
		SYUDPEvent e;
	...
	// Unicode(WCHAR) to ANSI(char)
	char *pAnsi = NULL;
	UnicodeToAnsi(SYUDPBROADCAST_KEY, &pAnsi);
	sendto(pSYSocket->Socket, (const char*)pAnsi, (int)strlen(pAnsi), 0, (SOCKADDR*)&boradcast_addr, sizeof(boradcast_addr));
	delete pAnsi;
	DWORD begin_time = GetTickCount();

	while(true)
	{
		if ((int)(GetTickCount() - begin_time) >= 1000*pSYSocket->_timerOut){
			//Terminate
			break;
		}
		...
		int bytes_recieved = recvfrom(pSYSocket->Socket, recv_data, SYUDPSOCKET_BUFFER_LENGTH, 0, (sockaddr*)&client_addr, &len) ;  
		if (bytes_recieved>0){
			//for this error with (bytes_recieved == -1)
			//Run-Time Check Failure #2 - Stack around the variable 'recv_data' was corrupted.
			recv_data[bytes_recieved] = '\0';
		}
		...
		{	
			// ANSI(char) to Unicode(WCHAR)
			wchar_t *pUni = NULL;
			AnsiToUnicode(recv_data, &pUni);
			// find key
			std::wstring tmp = pUni;
			std::size_t pos1 = tmp.find(L"<key>");
			std::size_t pos2 = tmp.find(L"</key>");
			if (pos1 == std::string::npos || pos2 == std::string::npos)
			{
				break;
			}
			pos1+=5;
			std::wstring key = tmp.substr(pos1, pos2-(pos1));
			//find name
			tmp = pUni;
			pos1 = tmp.find(L"<name>");
			pos2 = tmp.find(L"</name>");
			if (pos1 == std::string::npos || pos2 == std::string::npos)
			{
				break;
			}
			pos1+=6;
			std::wstring name = tmp.substr(pos1, pos2-pos1);
			//Event
			if (wcscmp(SYUDPBROADCAST_KEY, key.c_str()) == 0)
			{
				
				SYUDPEvent e;
				ZeroMemory(&e, sizeof(e));
				e.Status = SYUDPSOCKET_FIND_SERVICE;
				e.iLen = bytes_recieved;
				memcpy(e.szData, recv_data, sizeof(recv_data) );				
				e.Port = ntohs(client_addr.sin_port);	
				...
				// ANSI(char) to Unicode(WCHAR)
				wchar_t *pUnicode = NULL;
				AnsiToUnicode(inet_ntoa(client_addr.sin_addr), &pUnicode);				
				wsprintf(e.wszMsg, L"%s:%s", pUnicode, name.c_str());		
				pSYSocket->OnEvent(pSYSocket, e);	
				free(pUnicode);

				char szBuf[256];
				memset(szBuf, 0, sizeof(szBuf));
				sprintf_s(szBuf, "\nServer to Client(%s , %d) said : %s\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port), recv_data);
				OutputDebugStringA(szBuf);
	...
	OutputDebugStringA("UDP Client is Terminate\n");
	/*
	shutdown(pSYSocket->Socket, SD_BOTH);
	closesocket(pSYSocket->Socket);
	Sleep(1000);
